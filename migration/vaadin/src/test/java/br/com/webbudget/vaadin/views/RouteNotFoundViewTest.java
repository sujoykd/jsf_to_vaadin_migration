...
class RouteNotFoundViewTest {
    ...
    @Test
    void view_has_404_status_code_heading() {
        var view = new RouteNotFoundView();
        assertThat(view.statusCode.getText()).isEqualTo("404");
    }

    @Test
    @SuppressWarnings("unchecked")
    void setErrorParameter_returns_404_status_code() {
        var view = new RouteNotFoundView();
        var event = mock(BeforeEnterEvent.class);
        var param = (ErrorParameter<NotFoundException>) mock(ErrorParameter.class);
        when(param.getException()).thenReturn(new NotFoundException("not found"));
    ...
    void i18n_messages_exist_in_en_US() {
        ...
        var msg = ms.getMessage("404.message", null, Locale.of("en", "US"));
        assertThat(msg).contains("page");

        var details = ms.getMessage("404.details", null, Locale.of("en", "US"));
    ...
    void i18n_messages_exist_in_pt_BR() {
        ...
        var msg = ms.getMessage("404.message", null, Locale.of("pt", "BR"));
        assertThat(msg).isNotBlank();

        var details = ms.getMessage("404.details", null, Locale.of("pt", "BR"));
        assertThat(details).isNotBlank();
    }
}
