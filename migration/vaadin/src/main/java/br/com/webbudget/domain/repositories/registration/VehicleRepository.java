...
public interface VehicleRepository extends LazyDefaultRepository<Vehicle> {
     ...
     */
    @Override
    default Specification<Vehicle> getEntityStateSpec(Boolean active) {
        return (root, query, cb) -> cb.equal(root.get(Vehicle_.active), active);
    }
     ...
     */
    @Override
    default Specification<Vehicle> getFilterSpecification(String filter) {
        final String pattern = likeAny(filter).toLowerCase();
        return (root, query, cb) -> cb.or(
            cb.like(cb.lower(root.get(Vehicle_.identification)), pattern),
            cb.like(cb.lower(root.get(Vehicle_.brand)), pattern),
            cb.like(cb.lower(root.get(Vehicle_.model)), pattern),
            cb.like(cb.lower(root.get(Vehicle_.licensePlate)), pattern)
        );
    }
}
