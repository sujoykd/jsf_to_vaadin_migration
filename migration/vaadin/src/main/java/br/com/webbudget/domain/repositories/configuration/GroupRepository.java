...
public interface GroupRepository extends LazyDefaultRepository<Group> {
     ...
     */
    @Override
    default Specification<Group> getEntityStateSpec(Boolean active) {
        return (root, query, cb) -> cb.equal(root.get(Group_.active), active);
    }
     ...
     */
    @Override
    default Specification<Group> getFilterSpecification(String filter) {
        final String pattern = likeAny(filter).toLowerCase();
        return (root, query, cb) -> cb.like(cb.lower(root.get(Group_.name)), pattern);
    }
}
